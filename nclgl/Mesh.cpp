...
Mesh* Mesh::GenerateQuad()
{
    ...
    mesh -> textureCoords = new Vector2[mesh-> numVertices];
	mesh -> colours = new Vector4[mesh-> numVertices];
	mesh -> normals = new Vector3[mesh -> numVertices]; // Init new var !
	mesh -> tangents = new Vector4[mesh -> numVertices]; // Init new var !
	...
	for (int i = 0; i < 4; ++i) {
		...
		mesh -> normals[i] = Vector3(0.0f, 0.0f, -1.0f); // New !
		mesh -> tangents[i] = Vector4(1.0f, 0.0f, 0.0f, 1.0f); // New !
